...
import { mostrarInformacionClientes, nuevoCliente } from '../../../redux/slice/mantenimientos/clientesSlice';
...
export const ClientesFiltros = () => {
    ...
    const handleClickMostrar = ()=>{
       dispatch(mostrarInformacionClientes(Pub_Esta_Codigo )) 
    }   

    const handleClick_nuevo= ()=>{          
        dispatch(nuevoCliente())
    }

    useEffect(() => {
        dispatch(mostrarInformacionClientes('001')) 
  },  [dispatch]) 
  ...
  return (    
    <Card> 
        <CardContent>
            <div>
                <p className='titleFormAlternative'> MANTENIMIENTO DE CLIENTES </p>
            </div>
