...
import { save, update, validaciones } from '../../../redux/slice/mantenimientos/serviciosSlice';
...
const useStyles = makeStyles((theme) => ({
  ...
  export const ServicioRegistroForm = () => {    
  ...
  const { id_Servicio, nombre_Servicio, abreviatura_Servicio, id_Estado   } = formParams;
  ...
  const handleSave =async ()=>{
    if (flag_modoEdicion === true) {     ///-- edicion
        ...
        if (valor === false) return;
        dispatch(save({ ...formParams, nombre_Servicio : String(nombre_Servicio).trim() , usuario_creacion : id_usuarioGlobal} )); 
    }   
  }  
        ...
        <form className={classes.formControl} noValidate autoComplete="off">
            <Row className='p-2'  >
                <Col lg={ 12 } >
                        ...
                        <Row>
                            <Col lg={6} >   
                                <TextField  label="Codigo Interno" disabled name="id_Servicio" value= { id_Servicio}   onChange={ handleInputChange } />
                            </Col>
                        </Row>  
                        <Row>
                            <Col md={12} lg={12}  >   
                                <TextField  label="Servicio"  inputProps={{ maxLength: 100 }}  name="nombre_Servicio" value= { nombre_Servicio}   onChange={ handleInputChange } />
                            </Col>
                        </Row> 
                        <Row>
                            <Col md={6} lg={6}  >   
                                <TextField  label="Abreviatura"  inputProps={{ maxLength: 20 }}   name="abreviatura_Servicio" value= { abreviatura_Servicio}   onChange={ handleInputChange } />
                            </Col>
                        </Row> 
                         <br/>
                        <Row  >
